    ...
    <main className="px-5 md:px-10 lg:px-14 grid gap-5">
      ...
      <section className="w-full grid grid-cols-1 grid-rows-[1fr,auto] lg:grid-cols-[70%,30%] lg:grid-rows-1 gap-5">
        ...
        <div className="hidden md:flex flex-row bg-gray-100 items-center p-2 bg-[#E8E8E899]">
            <button 
              className={`py-2 px-4 ${activeStatus === "1" ? "bg-white" : "hover:bg-white"}`} 
              onClick={() => fetchOrderHistory("1")}
            >
              On Progress
            </button>
            <button 
              className={`py-2 px-4 ${activeStatus === "2" ? "bg-white" : "hover:bg-white"}`} 
              onClick={() => fetchOrderHistory("2")}
            >
              Sending Goods
            </button>
            <button 
              className={`py-2 px-4 ${activeStatus === "3" ? "bg-white" : "hover:bg-white"}`} 
          ...
          {history.length > 0 ? (
            <div className="grid grid-cols-1 gap-4">
              {history.map(({ id, img_product, order_number, created_date, product_price, status }) => (
                <div
                  key={id}
                  className="grid grid-cols-[1fr,1fr] md:grid-cols-[auto,1fr,1fr,1fr,1fr] gap-5 p-3 items-start border"
                >
                  <div className="w-full h-full grid items-center">
                    <img className="w-full h-full md:w-[100px] md:h-[100px]" src={img_product} alt="Product" />
                  </div>
                  ...
                  <div className="px-2 grid md:justify-center">
                    ...
                    <div>
                      ...
                      <Link to={`/order/${id}`} className="underline hidden md:block lg:hidden text-oren text-base text-primary">
                        View Order
                      </Link>
                      <Link to={`/order/${id}`} className="whitespace-nowrap underline block md:hidden lg:block text-oren text-base text-primary">
                        View Order Details
                      </Link>
                    </div>
                  </div>
                  ...
                  <div className="grid md:justify-center">
                    ...
                    <div className="font-bold text-base text-[#0B132A]">IDR {product_price}</div>
                  </div>
